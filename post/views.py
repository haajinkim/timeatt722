...
class ApplyView(APIView):
    ...
    def get(self, request):
        ...
        status_get = Jobstatus.objects.get(status=status)
        job_status_all = JobPostActivity.objects.filter(job_post_status_id=status_get.id)
        print(job_status_all)
        return Response(JobPostActivitySerializer(job_status_all, many=True).data)
    ...
    def post(self, request):
        ...
        if apply_serialzer.is_valid():
            apply_serialzer.save(job_post_status_id=2)
            return Response(status=status.HTTP_200_OK)
